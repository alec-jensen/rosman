...
class RosmanConfig:
    ...
    locked_pip_packages: list[str] = field(default_factory=list)
...
def write_lock(
    ...
    payload = {
        ...
        "pip_packages": sorted(set(pip_packages or [])),
    }
    header = (
...
def _validate_lock_package_list(data: dict, key: str, path: Path) -> list[str]:
...
def _read_lock(config_path: Path, ros_distro: str) -> tuple[list[str], list[str]]:
    """Returns (apt_packages, pip_packages)."""
    path = lock_path(config_path)
    if not path.is_file():
        return [], []
    try:
    ...
    apt_packages = _validate_lock_package_list(data, "apt_packages", path)
    pip_packages = _validate_lock_package_list(data, "pip_packages", path)
...
def load_config(path: Path) -> RosmanConfig:
    ...
    config.locked_apt_packages, config.locked_pip_packages = _read_lock(path, config.ros_distro)
    return config
